...
class Message {
	...
	public function getConvos() {
		...
		foreach($convos as $username) {
			...
			$return_string .= "<a href='messages.php?u=$username'> <div class='user_found_messages'>
								<img src='" . $user_found_obj->getProfilePic() . "' style='border-radius: 25px; height:50px; width:50px; margin-right: 8px;'>
								" . $user_found_obj->getFirstAndLastName() . "
								<span class='timestamp_smaller style_s' id='grey'> </br>" . $latest_message_details[2] . "</span>
	...
	public function getConvosDropdown($data, $limit) {
		...
		echo '<div class="msg_top">Messages</div>';
		foreach($convos as $username) {
			...
			$style = ($row['opened'] == 'no') ? "background-color: var(--drop-hov);" : "";
			...
			$return_string .= "<a href='messages.php?u=$username'> 
								...
								<img src='" . $user_found_obj->getProfilePic() . "' style='border-radius: 5px; margin-right: 8px; height:55px; width:55px; border-radius: 50px; margin-left:8px;'>
								" . $user_found_obj->getFirstAndLastName() . "
								<span class='timestamp_smaller' id='grey' style='font-size: 11px; display: block;'> " . $latest_message_details[2] . "</span>
								<p id='grey' style='margin: 0; '>" . $latest_message_details[0] . $split . " </p>
								</div>
								</a>";
		}
		...
		if($count > $limit)
			...
			$return_string .= "<input type='hidden' class='noMoreDropdownData' value='true'><hr><p style='text-align: center; color:grey;'>No more messages to load!</p>";
